...
class VoiceCommandDispatcher(
    ...
    fun dispatch(command: EmailCommandTool.Command) {
        ...
        when (command) {
            ...
            is EmailCommandTool.Command.CheckAvailability -> {
                // Let the natural Gemini voice handle the confirmation.
                // viewModel.checkAvailability(command.durationMinutes)
            }

            is EmailCommandTool.Command.ProposeTimeSlot -> {
                // Let the natural Gemini voice handle the confirmation.
                // viewModel.proposeTimeSlot(command.durationMinutes)
            }

            is EmailCommandTool.Command.ShowCalendar -> {
                viewModel.showCalendar()
            }

            is EmailCommandTool.Command.HideCalendar -> {
